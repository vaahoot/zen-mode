...
static int line_matches_domain(const char *line, const char *domain) {
    ...
    while (*p) {
        ...
        if (flen == dlen && memcmp(start, domain, dlen) == 0) return 1;
        if (flen == dlen + 4 && memcmp(start, "www.", 4) == 0
            && memcmp(start + 4, domain, dlen) == 0) return 1;
    }
    return 0;
}
...
void unblock_domain(const char *domain) {
    ...
    while (fgets(buf, sizeof(buf), in)) {
        int drop = strstr(buf, MARKER)
                   && (!domain || line_matches_domain(buf, domain));
